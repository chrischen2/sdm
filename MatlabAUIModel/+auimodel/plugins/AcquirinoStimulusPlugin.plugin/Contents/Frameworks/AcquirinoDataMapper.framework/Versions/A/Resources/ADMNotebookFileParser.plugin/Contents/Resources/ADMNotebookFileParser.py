...
class ADMNotebookFileParser(AbstractParser):
  ...
  # protocol comment marking start of a dynamic clamp epoch family.
  # string that follows should be parsed for params.
  DYN_CLAMP_START = '*Conductance clamp step, '
  ...
  def init(self):
    ...
    self.dateFormatter.initWithDateFormat_allowNaturalLanguage_("%m%d%y %I:%M:%S %p %Z", None)
    ...
    while token != None:
      ...
      if token.id == Token.Cell_Header:
        ...
        if cell_basenames.issuperset(set([basename])):
          raise Exception, 'Cell data file %s appears twice in notes.' % basename
      ...
      if token.id == Token.Family_Start:
        ...
        if token.id == Token.Family_Start:
          ...
          log.debug(token)
          raise Exception, 'Protocol comment must follow family starting at %s' % start_date
            
        family = EpochFamily.new()
        family.setStartDate_(start_date)
        family.setProtocolComment_(token.value.strip("*\n"))
        family.setEpochSettings_(NSMutableDictionary.dictionaryWithCapacity_(10))

        epochs = dict()
        parameters = dict()

        if token.value.startswith(ADMNotebookFileParser.DYN_CLAMP_START):
          params_string = token.value[len(ADMNotebookFileParser.DYN_CLAMP_START):]
          self.parseDynClampParams(params_string, parameters)
        ...
        family.setEpochNumberRange_(self.makeRangeFrom_to_(start_epoch_number,
          last_epoch_number))
  ...
  def parseDynClampParams(self, params_string, params):
    pairs = re.compile("(?<=\d)\s+(?=\w)").split(params_string)
  ...
  def parseEpochParameters(self, line, params):
    ...
    pattern = re.compile('([^:=]+\s*=\s*[-0-9.]+\s*(?:\w{1}(?:\s|$))?|[^:\s]+:)')

    cursor = 0
    match = pattern.search(line, cursor)

    while match:
      key, value = match.groups()[0].split('=')
      key = key.strip()
      value = value.strip()
    ...
    # used to grab trailing flags e.g. for gclamp experiment "OffStepInhG1"
    # you can have one more than one flag at the end of an epoch line
    # each will become setting with value "true"
    # note that confusion could result from units of
    # more than one character in a different param. (usually only "C" for celcius)
    ...
    while match:
      ...
      gclamp_match = re.match("^(ON|OFF)(exc|inh)_(.+)", key)

      # parse flags like ONexc_lo0, ONexc_hi_100_2, ONinh_hi0
      if gclamp_match:
        key = gclamp_match.group(2)
        value = gclamp_match.group(1) + "." + gclamp_match.group(3)
  ...
  def labelCell(self, cell):
      ...
      if len(fams) > 0 and len(notes) > 0 and fams[-1].startDate().compare_(notes[-1].timestamp()) < 0:
...
if ( __name__ == '__main__' ):
  ...
  parser = ADMNotebookFileParser.parserForFile_\
    ('/Users/dan/Desktop/081809B-notes.txt')
